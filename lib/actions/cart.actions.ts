...
export async function removeItemToCart(productId: string) {
  try {
    ...
    const exitingItem = cart.items.find((x) => x.productId === productId);
    if (!exitingItem) throw new Error("Item is not found to cart");
    if (exitingItem.qty === 1) {
      cart.items = cart.items.filter(
        (x) => x.productId !== exitingItem?.productId
      );
    } else {
      exitingItem.qty -= 1;
    }
    await updateCartPrices(cart.id, cart.items);
